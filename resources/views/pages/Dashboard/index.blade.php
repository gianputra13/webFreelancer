            ...
            <div class="col-span-4 text-right">
                <div @click.away="open = false" class="relative z-10 hidden mt-5 lg:block" x-data="{ open: false }">
                    ...
                    @if (auth()->user()->detail_user()->first()->photo != NULL)
                        <img src="{{ url(Strorage::url(auth()->user()->detail_user()->first()->photo)) }}" alt="photo profile" class="inline w-12 h-12 mr-3 rounded-full">
                    @else
                            ...
                            <tr class="text-gray-700">
                                <td class="w-1/3 px-1 py-5">
                                    <div class="flex items-center text-sm">
                                        ...
                                        @if (auth()->user()->detail_user()->first()->photo != NULL)
                                            <img src="{{ url(Strorage::url(auth()->user()->detail_user()->first()->photo)) }}" alt="photo profile" class="object-cover w-full h-full rounded-full">
                                        @else
                                            <img class="object-cover w-full h-full rounded-full text-gray-300" src="{{ url('https://randomuser.me/api/portraits/men/2.jpg') }}" alt="" loading="lazy" />
                                        @endif
                                        ...
                                        <div>
                                            <p class="font-medium text-black">{{ $order->user_buyer->name ?? '' }}</p>
                                                @if ($order->pesan_status_id == '1')
                                                    <p class="text-sm text-green-500">{{ $order->pesan_status->name ?? '' }}</p>
                                                @elseif ($order->pesan_status_id == '2')
                                                    <p class="text-sm text-yellow-500">{{ $order->pesan_status->name ?? '' }}</p>
                                                @elseif ($order->pesan_status_id == '3')
                                                    <p class="text-sm text-red-500">{{ $order->pesan_status->name ?? '' }}</p>
                                                @else
                                                    <p class="text-sm text-black">{{ $order->pesan_status->name ?? '' }}</p>
                                                @endif
                                        </div>
                                    </div>
                                </td>
